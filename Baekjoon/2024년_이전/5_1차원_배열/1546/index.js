...
if (condition(N)) {
    const sumFixScore = () => {
        ...
        arr.forEach((v) => sum += v / maxScore * 100);
        return sum / N;
    }

    const average = sumFixScore();
    console.log(average);
}
...
// const remainderArr = new Set(arr.filter((v) => condition(v)).map((v2) => v2 % 42));
// console.log(remainderArr.size);
