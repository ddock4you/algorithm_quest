/*
TODO 제목: 색종이
! 문제: 가로, 세로의 크기가 각각 100인 정사각형 모양의 흰색 도화지가 있다. 이 도화지 위에 가로, 세로의 크기가 각각 10인 정사각형 모양의 검은색 색종이를 색종이의 변과 도화지의 변이 평행하도록 붙인다.
...
(() => {
    const number = Number(input.splice(0, 1));
    const arrays = input.map((item) => item.trim().split(" "));
    let paper = Array.from(Array(100), () => Array(100).fill(0));
    let sum = 0;
    arrays.forEach((item) => {
        const x = Number(item[0]);
        const y = Number(item[1]);
        for (let i = x; i < x + 10; i += 1) {
            for (let j = y; j < y + 10; j += 1) {
                paper[i][j] += 1;
                if (paper[i][j] === 1) {
                    sum += 1;
                }
