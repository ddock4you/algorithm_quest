...
(() => {
    let fullText = "";
    const maximumArray = input.reduce((array, item) => {
        if (item.length <= 0 || item.length > 15) return array;
    ...
    for (let i = 0; i < maximumArray.length; i += 1) {
        for (let j = 0; j < input.length; j += 1) {
            if (!input[j][i] || !/^[A-Za-z0-9][A-Za-z0-9]*$/.test(input[j][i])) continue;
            fullText += input[j][i];
        }
    }
    console.log(fullText || null);
})();
