...
if(condition(N)) {
    const groupWordChecker = Array.reduce((acc, word) => {
        let confirmWord = [];
        const confirm = word.split('').every((v, i) => {
            if (v === word[i + 1]) return true;
            if (!confirmWord.includes(v)) {
                confirmWord.push(v);
                return true;
            }
            return;
